...
20. **Use `block` for flex rows, NOT `div`** — `div` elements ignore `_direction: row` and always stack vertically. Use `block` with `_direction: row` for horizontal layouts. Both `block` and `container` support flex direction; `div` does not. If you need items side-by-side, always use `block`.
